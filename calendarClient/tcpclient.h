...
public:
    ...
    // 0820 캘린더 목록을 캐시로 저장하는 함수
    const QStringList& calNameList() const { return m_cacheCalendars; }
    const QList<CalendarInfo>& calTotalList() const { return m_cacheTotalCalendars;}
    // 0820 초대 전송하는 함수
    void sendInviteRequest(const QString& inviteeId, int calId);
...
signals:
    ...
    void inviteCalFailed(const QString& reason);
    void calendarListDirty();
    void eventsDirty(int calId);
...
private:
    ...
    QStringList m_cacheCalendars;
    QList<CalendarInfo> m_cacheTotalCalendars;
