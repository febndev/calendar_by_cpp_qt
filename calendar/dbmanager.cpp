...
// 테스트 코드용 함수 정의 시작
bool DBManager::insertTestUser(int id,
                               ...
                               const QString& role)
{
    ...
    QSqlQuery query;
    //SQL 준비문 : named placeholder(:id 등)을 사용한 prepared statement -> 이게 뭔말이람.
    query.prepare(R"(
    INSERT INTO users (id, userid, pw, username, useremail, role, created_at)
    VALUES (:id, :userid, :pw, :username, :useremail, :role, NOW())
)");
